...
| `internal/config/gitignore/`   | Git ignore pattern matching, config-scoped collection, and directory reachability                                              | Staged JS/TS catalogs carry a filesystem-independent cursor through their existing walk, pruning Git-inaccessible subtrees and freezing observed patterns for lint-target admission; low-level pre-resolved configs and the LSP workspace fallback reuse the direct collector. Pure pattern projection is provided by `internal/utils/gitignore`; collection and target-admission policy stay here                                          |
...
| `internal/program/loader/`     | Run-scoped Program construction and lint-target binding for CLI/API                                                            | Consumes target-owned lint plans, selects configured projects for matched targets in every CLI/API mode, binds targets by governing-config order and physical identity, and returns one backend-agnostic Program sequence plus its execution projection. Construction choices and ts-go compatibility details remain private; LSP wraps session-owned Programs directly and does not use this request-scoped loader                         |
...
| `packages/vscode-extension/`   | VS Code extension for IDE integration                                                                                          | Activation first checks whether `rstack.rstack` with `rstack.rslint.enable` on takes over; if so, this extension stands down and shows only its migration status bar item. Otherwise it resolves the nearest project-local `@rslint/core` per open document, launches that installation's `cmd/rslint --lsp`, serves reverse config/plugin requests, and routes diagnostics/code actions to the document's selected runtime                 |
...
`gitignore` shares pure pattern parsing: `ParseText` preserves the configuration
collector's existing glob projection, while `NewMatcher` and
`NewMatcherFromText` adapt explicit pattern lists and ignore-file text to the
...
2. **Program Loading**: `internal/config/target` freezes targets, their governing config owners, and path identities without consulting Programs. `config.FileConfigResolver` matches and merges each file's configuration once. `config.ResolveProjectPolicy` projects service/root/reset options from that same immutable result; it performs no matching, filesystem lookup, or project construction. CLI/API use `config/lint.Resolver` both before construction and after source binding, sharing its file and matched-shape caches. LSP consumes its existing document snapshot's resolved config. Node only evaluates and serializes values; presets remain ordinary reusable values.
   ...
   The loader appends selected config metadata to its existing project plan before construction. Config parsing uses a metadata-only host; source snapshots are bound only when Program construction has chosen its serial or parallel filesystem view. Ordinary and service modes share scheduling, source snapshots and one target-binding pass. Complete service Programs use source references and remain distinct from explicit Programs with different construction options. Sparse per-target candidates preserve scoped root choices and gaps; service candidates never leak into another target's ordinary declaration list.

   LSP runs the same metadata discovery against its current editor overlay before requesting a Program. Editor URI decoding uses tsgo's drive-letter spelling for overlays and file lifecycle events; document identity and protocol responses retain the original URI. Normal diagnostics and speculative fixes share the complete binding policy: disabled or unmatched targets use a source-only generation and cannot borrow a Session's default module graph. Program acquisition receives the selected parsed config, rather than only its path. Service reuse checks the actual Session Program's construction inputs and referenced config snapshots; a mismatch uses the existing standalone store with the selected metadata. Listed non-TS roots may also require the service construction mode. A missing source in an otherwise compatible selected Program remains an error.

   Existing watcher coverage owns standalone cache invalidation. Metadata already observed by a request also constrains resident Program reuse, including reference changes with an unchanged root config. Construction hosts retain only immutable parsed reference snapshots; unknown references still use the compiler host's normal reads and failed-lookup tracking. Metadata observed before watcher coverage stabilized cannot become resident. Speculative fixes keep their private source overlays and do not mutate the editor Session.
...
`internal/output.Diagnostic`, computes report counts and one outcome, and then
builds the completed `internal/output.Report`. API projects to its 1-based
structured wire model and flat UTF-16 edit offsets, and LSP projects to 0-based
LSP positions while retaining its stale-generation and code-action lifecycle.
...
Rslint's runtime configuration follows ESLint flat config semantics (an array
of config entries) and is loaded from JavaScript or TypeScript modules:
...
Runtime loading of `rslint.json` and `rslint.jsonc` is not supported. `--init`
remains the isolated migration entry point: when it finds a legacy JSON/JSONC
file, it parses that file, writes the recommended JS/TS module config, and
...
Runtime configuration uses staged JS/TS module catalogs:
...
uses only the selected JS/TS module.
...
- Extension activation makes a one-time stand-down decision through
  ...
  `Extension`, `RuntimeManager`, and LSP startup are skipped entirely. A live
  language client cannot be added or removed safely after activation, so later
  Rstack state changes only switch the notice to a reload prompt. Whichever
  extension activated the Rslint language server retains ownership of it.
...
- Only a fully committed Go/Node snapshot replaces a usable last-good
  ...
  boundaries, keeping the LSP alive without linting through broken subtrees. A
  Node commit retains one rollback predecessor: if the commit
...
tsconfig-derived state refresh. `document_sync.go` owns the
...
LSP explicitly stages and commits an empty plugin-host state (an empty load
batch followed by zero-ID activation) plus an empty-rule workspace fallback
whose only policy is the transaction's frozen `.gitignore` view. That empty
catalog is not a usable module-config last-good boundary: if a newly created
config is broken, LSP commits an unavailable boundary for it rather than
linting through the failed owner.

Low-level API and WASM callers may still send an already-resolved serialized
`config`; JSON there is a wire representation of the typed model, not a config
file lookup. The only disk JSONC parser for rslint configuration belongs to the
`--init` migration path.
...
- `.gitignore` is injected as a global-ignore entry through the shared
  ...
  directory chains within each governing config. Explicit module-config CLI
...
- when the client supports dynamic file-watch registration, Go watches
  ...
  workspace/descendant JS/TS configs and dependency lockfiles;
  ...
  session, but those forwarded config events do not start a second fresh config
  transaction. Create/change/delete events rebuild the frozen config/ignore snapshot and
  refresh open-document diagnostics
- the VS Code extension preserves last-good JS/TS configs during reloads; a
  newly unavailable config with no usable module-config ancestor contributes
  an empty boundary that suppresses lint in that authored subtree. A normal
...
- automatic Rslint config discovery selects the nearest loadable lint config; explicit config selection uses that config directly. Matching entries determine service, root and reset options. Service discovery starts at the lexical source location; ordinary project declarations retain owner order and prefer the first direct-root project, then the first import-containing Program
- omitting both project settings retains the config-directory `tsconfig.json` fallback. Effective `project: false` or `null` disables the target's ordinary binding. An empty array contributes no paths and suppresses fallback only when no other declaration supplies paths; `projectService: true` instead discovers configured ownership; unowned files use source-only gap linting. Built-in TypeScript presets do not set either project option, matching upstream preset composition. `tsconfigRootDir` is an absolute ancestor-search boundary, not a forced project, and defaults to the governing config directory (API cwd for inline-only configuration)
...
4. **Lint Target Plan**: Go resolves a stable target set from CLI/API scope, the implicit default baseline, explicit config `files`, global ignores, and `.gitignore`. Ordinary explicit-project `--type-check` keeps its existing build-before-scan order; `--type-check-only` skips the scan. Service/root/reset options require effective target config first
5. **Project Loading**: `program/loader.Session` accepts raw project declarations, frozen targets, and sparse service/root/reset policies projected from the shared file resolver. It assembles one project plan, preserves the invocation's broad/focused/program-wide construction scope, and adds service-selected config metadata before construction. Explicit and service construction modes remain separate; per-target candidate lists carry scoped root choices and explicit gaps
6. **Target Binding**: the loader locates the selected project's AST by lexical or frozen canonical identity without changing project policy or target scope. Disabled and unbound targets, including service misses, receive source-only Programs. They cannot borrow a Program selected by another target's parser policy. CLI/API receive one Program sequence and exact parallel target projections; they never construct compiler hosts or implement ownership
...
4. **Staged catalog discovery and Program creation**
   ...
   - Discovery commits only the config catalog in stable order. Lint then
     runs the separate target walker. Plain explicit-project type-check-only
     skips that walk; per-file service/root/reset options require it. Focused
     construction screens direct roots before probing import membership.
     Program-wide type checking retains complete declared projects and adds
     service-selected projects; lint binding does not narrow their diagnostics.
...
6. **Program source identity index** (`internal/program/loader`)
   ...
   - Independent binding realpath lookups use `core.NewWorkGroup`;
     `--singleThreaded` runs the same work serially.
   - A separate operation-scoped projection freezes roots from the actual
     type-capable Programs selected for the run only when the default CLI
     summary needs them. Frozen lint-target identities seed the pass; every
     remaining root uses the VFS's authoritative per-file realpath operation
     through one bounded worker pool. It never enumerates a directory or
     infers a file identity from its parent, so work is bounded by the
     requested roots and remains valid for overlay VFS implementations. The
     result is deduplicated in declaration order. This complete projection is
...
- **Report Root Identities Are Demand-Scoped**: only the default CLI format asks the loader to freeze roots from the actual type-capable Programs selected for the run, before core execution. The loader reuses target hints and resolves only the remaining root paths through one bounded per-file identity pass; the command immediately reduces the frozen result to a file count. Machine formats, autofix generations, `Program` caches, and report rendering retain none of this identity state. As with the rest of a loader Session generation, this assumes the filesystem namespace is stable for the run; the shared request-local VFS may be warmed by the projection, and concurrent external namespace mutation is not a supported consistency boundary.
- **Worker-Local JavaScript Selector Compilation**: each third-party-plugin worker keeps a 2,048-entry FIFO cache keyed by the exact non-bare selector string. Entries contain only the immutable canonical esquery parse result, specificity, conservative anchor types, or an invalid-selector message; per-file contexts, listeners, options, ASTs, source code, and match results remain outside the cache. Bare node-type selectors retain their direct dispatch path, and replacing the worker naturally discards the cache.
...
- **Go Plugin Rules**: add a package under `internal/plugins/<plugin>/rules/<rule_name>/` and append the rule var to that plugin's `all.go`; a new bundled plugin also needs one aggregation entry in `internal/rules/all.go` and any accepted string declaration aliases in `internal/config/plugin_declarations.go`
...
│            │                                                                 │
│            ▼                                                                 │
│  Shared File Config Resolver: matched service/root/reset options             │
│            │                                                                 │
│            ▼                                                                 │
│  Program Loader: declared/service candidates, build, then source binding      │
│            │                                                                 │
│            ▼                                                                 │
│  Unified Program Sequence + Target / Path Projections                        │
│            │                                                                 │
│            ▼                                                                 │
│  Reuse the Same Immutable File Config and Enabled Rules                      │
│            │                                                                 │
│            ▼                                                                 │
